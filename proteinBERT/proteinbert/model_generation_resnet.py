...
class FinetuningModelGenerator(ModelGenerator):
    ...
    def create_model(self, seq_len, freeze_pretrained_layers = False):
        ...
        ##change add cnn or lstm
        elif self.output_spec.output_type.is_binary:
            print('last_hidden_layer',last_hidden_layer.shape)
            #last_hidden_layer (None, 15599)
            reduce_layer = keras.layers.Dense(18432, activation = 'sigmoid')(last_hidden_layer)
            # 加载预训练的ResNet模型，并冻结其权重（可选）
            resnet_model = ResNet50(weights='imagenet', include_top=False, input_shape=(32,192,3))
            resnet_model.trainable = True  # 可选，冻结预训练权重

            # 在last_hidden_layer后面连接ResNet模型和MLP层
            x = Reshape((32,192,3))(reduce_layer)
            x = resnet_model(x)
            x = Flatten()(x)
