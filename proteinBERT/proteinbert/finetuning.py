...
def evaluate_by_len(model_generator, input_encoder, output_spec, protein_ID, seqs, raw_Y, start_seq_len = 512, start_batch_size = 32, increase_factor = 2):
    
    assert model_generator.optimizer_weights is None
    
    dataset = pd.DataFrame({'protein_ID':protein_ID,'seq': seqs, 'raw_y': raw_Y})
    ...
    for len_matching_dataset, seq_len, batch_size in split_dataset_by_len(dataset, start_seq_len = start_seq_len, start_batch_size = start_batch_size, \
        ...
        X, y_true, sample_weights,test_id = test_encode_dataset(len_matching_dataset['protein_ID'],len_matching_dataset['seq'], len_matching_dataset['raw_y'], input_encoder, output_spec, \
    ...
    return X_test,y_preds,y_trues,results, confusion_matrix


#####
def evaluate_by_len_prediction(model_generator, input_encoder, output_spec, protein_ID, seqs, start_seq_len = 512, start_batch_size = 32, increase_factor = 2):
    
    assert model_generator.optimizer_weights is None
    
    dataset = pd.DataFrame({'protein_ID':protein_ID,'seq': seqs})
    ...
    for len_matching_dataset, seq_len, batch_size in split_dataset_by_len(dataset, start_seq_len = start_seq_len, start_batch_size = start_batch_size, \
        ...
        X,test_id = test_encode_dataset_prediction(len_matching_dataset['protein_ID'],len_matching_dataset['seq'], input_encoder, output_spec, \
                seq_len = seq_len, needs_filtering = False)
...
def test_encode_dataset(protein_ID, seqs, raw_Y, input_encoder, output_spec, seq_len = 512, needs_filtering = True, dataset_name = 'Dataset', verbose = True):
    ...
    if needs_filtering:
        ...
        seqs = dataset['seq']
        raw_Y = dataset['raw_Y'] 
        test_id  = dataset['protein_ID']
        
    test_id  = protein_ID
    X = input_encoder.encode_X(seqs, seq_len)
    Y, sample_weigths = encode_Y(raw_Y, output_spec, seq_len = seq_len)
    return X, Y, sample_weigths,test_id
###
def test_encode_dataset_prediction(protein_ID, seqs, input_encoder, output_spec, seq_len = 512, needs_filtering = True, dataset_name = 'Dataset', verbose = True):
